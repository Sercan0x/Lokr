...
const removeFromWhitelist = async (address) => {
  ...
  const withdrawAll = async () => {
    if (!window.confirm('Are you sure you want to withdraw all USDC from your vault?')) {
      return;
    }
    ...
    try {
      setIsLoading(true);
      const vaultContract = new Contract(vaultAddress, VAULT_ABI, signer);
      const tx = await vaultContract.withdrawAll(account);
      await tx.wait();
  ...
  const changeOwner = async () => {
    ...
    if (!window.confirm(`Are you sure you want to transfer ownership to ${newOwnerAddress}?`)) {
      return;
    }
    ...
    try {
      setIsLoading(true);
      const vaultContract = new Contract(vaultAddress, VAULT_ABI, signer);
      const tx = await vaultContract.transferOwnership(newOwnerAddress);
      await tx.wait();
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <button
                    onClick={withdrawAll}
                    disabled={isLoading}
                    className="w-full px-6 py-3 bg-gray-700 rounded-xl font-semibold hover:bg-gray-600 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
                  >
            ...
            <div className="space-y-8">
              ...
              <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-6 space-y-4">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-gray-900/50 border border-orange-700 rounded-xl focus:border-orange-500 focus:outline-none"
                />
